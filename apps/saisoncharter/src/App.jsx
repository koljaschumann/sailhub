...
function Navigation() {
  ...
  const navItems = [
    { path: '/', label: 'Buchen', icon: Icons.plus },
    { path: '/boote', label: 'Bootskalender', icon: Icons.calendar },
    { path: '/buchungen', label: 'Meine Buchungen', icon: Icons.list },
      ...
      <div className="max-w-6xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isDark ? 'bg-emerald-500/10' : 'bg-emerald-100'}`}>
              <span className={`w-6 h-6 ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>
                {Icons.sailboat}
              </span>
            </div>
            <div>
    ...
    <div className={`min-h-screen ${isDark ? 'bg-navy-900' : 'bg-light-bg'}`}>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <Routes>
          ...
          <Route path="/boote" element={<BoatCalendarPage />} />
          <Route path="/buchungen" element={<MyBookingsPage />} />
