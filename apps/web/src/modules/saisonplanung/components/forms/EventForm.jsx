...
import { useTheme, Button, Icons, useToast } from '@tsc/ui';
import { useAuth } from '@tsc/supabase';
import { motorboats, getMotorboatsSortedByPriority, boatClasses, boatClassCategories } from '@tsc/data';
...
export function EventForm({ onSuccess, editEvent = null }) {
  ...
  const [formData, setFormData] = useState({
    ...
    name: editEvent?.name || '',
    boatClassId: editEvent?.boatClassId || currentBoatClassId || '',
  ...
  const sortedMotorboats = currentBoatClassId
    ? getMotorboatsSortedByPriority(currentBoatClassId)
    : motorboats;
  ...
  const validate = () => {
    ...
    if (!formData.boatClassId) {
      newErrors.boatClassId = 'Bootsklasse ist erforderlich';
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <select
          name="boatClassId"
          value={formData.boatClassId}
          onChange={handleChange}
          disabled={!canEdit}
          className={inputClass}
        >
          <option value="">Bitte wählen...</option>
          {boatClassCategories.map(category => (
            <optgroup key={category.id} label={category.name}>
              {boatClasses
                .filter(bc => bc.category === category.id)
                .map(bc => (
        ...
        {errors.boatClassId && <p className="mt-1 text-xs text-coral">{errors.boatClassId}</p>}
      </div>
        ...
        <div>
          ...
          >
            ...
            {sortedMotorboats.map(boat => {
              const hasPriority = boat.priority.includes(currentBoatClassId);
              return (
          ...
          {(currentBoatClassId === '29er' || currentBoatClassId === 'j70') && (
      ...
      <div className="flex gap-3 pt-2">
        <Button type="submit" className="flex-1" disabled={!canEdit || isSubmitting}>
          {isSubmitting ? 'Speichern...' : editEvent ? 'Speichern' : 'Hinzufügen'}
        </Button>
      </div>
