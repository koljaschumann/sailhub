...
function Navigation() {
  ...
  const navItems = [
    { path: '/', label: 'Übersicht', icon: Icons.chart },
    { path: '/rangliste', label: 'Rangliste', icon: Icons.trophy },
    { path: '/auszeichnungen', label: 'Auszeichnungen', icon: Icons.medal },
  ];
      ...
      <div className="max-w-6xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isDark ? 'bg-amber-500/10' : 'bg-amber-100'}`}>
              <span className={`w-6 h-6 ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>
                {Icons.trophy}
              </span>
            </div>
            <div>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-2 px-4 py-2.5 rounded-t-lg text-sm font-medium transition-colors whitespace-nowrap ${
                    isActive
                      ? isDark
                        ? 'bg-navy-800 text-amber-400 border-b-2 border-amber-400'
    ...
    <div className={`min-h-screen ${isDark ? 'bg-navy-900' : 'bg-light-bg'}`}>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <Routes>
          ...
          <Route path="/rangliste" element={<RankingsPage />} />
          <Route path="/auszeichnungen" element={<AwardsPage />} />
        </Routes>
      </main>
