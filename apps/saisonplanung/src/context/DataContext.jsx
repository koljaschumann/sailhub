...
export function DataProvider({ children }) {
  ...
  const loadDemoData = async () => {
    ...
    try {
      ...
      if (devMode) {
        // Local-only demo data (no Supabase)
        const localEvents = demoEvents.map((e, index) => ({
          id: `demo-${index + 1}`,
          ...e,
          assignedMotorboat: e.requestedMotorboat,
        ...
        // Set deadline locally
        const futureDate = new Date();
        futureDate.setMonth(futureDate.getMonth() + 1);
        setDeadlineState(futureDate.toISOString().split('T')[0]);
